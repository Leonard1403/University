...
void populeazaDOVidIdentic(DO& doc, int cMin, int cMax) {
	...
	TValoare val;
	for (int i = 0; i < n; i++) {
		//std::cout << chei[i] << " " << chei[i] << '\n';
		//std::cout << doc.adauga(chei[i], chei[i]) << '\n';
		val = doc.adauga(chei[i], chei[i]);
		doc.afisare(chei[i]);
		std::cout << "\n" << val << " " << NULL_TVALOARE << '\n';
		assert(val == NULL_TVALOARE);
	}
}
...
void testAdauga() {
	...
	cout << "Adauga si cauta crscator passed\n";
	testAdaugaSiCauta(desc);
	cout << "Adauga si cauta descrescator passed\n";
}
...
void testFNou() {
	...
	d.adauga(9, 15);
	d.adauga(10, 15);
	//d.afisare(cMin, cMax+1);
	TValoare val;
	val = d.ceaMaiFrecventaValoare();
	//std::cout << val;
	assert(val == 15);
	d.sterge(8);
	d.sterge(9);
...
void testIterator() {
	testIterator(cresc);
	std::cout << "Bun test crescator.\n";
	testIterator(desc);
	std::cout << "Bun test descrescator\n";
}

void testAllExtins() {
	
	testCreeaza();
	std::cout << "Test de creare passed\n";
	//testAdauga();
	//std::cout << "Test de adauga passed\n";
	testCauta();
	std::cout << "Test de cauta passed\n";
	testSterge();
	std::cout << "Test de sterge passed\n";
	testIterator();
	std::cout << "Test de iterator passed\n";
	testCantitativ();
	std::cout << "Test de cantitativ passed\n";

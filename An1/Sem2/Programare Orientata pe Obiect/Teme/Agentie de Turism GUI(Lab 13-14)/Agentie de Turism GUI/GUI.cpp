...
void ConsoleGUI::initializeGUIComponents() {
	...
	//CRUD
	btnCRUD = new QPushButton("Afisare Wishlist");
	lyLeftleft->addWidget(btnCRUD);

	//DRAW
	btnDRAW = new QPushButton("Desenare Wishlist");
	...
	this->tableOferte = new QListWidget;
...
void ConsoleGUI::reloadOferteList(vector<Turism> oferte) {
	...
	//	lineNumber++;
	//}
	this->tableOferte->clear();
	for (auto& oferta : oferte)
	{
		this->tableOferte->addItem(oferta.str().c_str());
	}
}
...
void ConsoleGUI::connectSignalsSlots() {
	...
	//ADD
	QObject::connect(btnAddWishlist, &QPushButton::clicked, this, &ConsoleGUI::gui_AddWishlist);
	QObject::connect(btnAddRandom, &QPushButton::clicked, this, &ConsoleGUI::gui_AddWishlistmRandom);

	QObject::connect(btnGoleste, &QPushButton::clicked, [&]() {
		srv.golesteWishlist();
		QMessageBox::information(this, "Info", QString::fromStdString("Lista de oferte din program a fost golita!"));
		reloadOfertaListFromWishlist(srv.getAllWishlist());
		});
	...
	QObject::connect(btnCRUD, &QPushButton::clicked, [this] {
		auto wishlist = new WishlistCRUD_GUI(srv.get_wishlist(), srv);
		wishlist->run();
		});

	//afisare window DRAW
	QObject::connect(btnDRAW, &QPushButton::clicked, [this] {
		auto wishlist = new WishlistReadOnlyGUI(srv.get_wishlist());
		wishlist->show();
		});
}
...
void ConsoleGUI::reloadOfertaListFromWishlist(vector<Turism> oferte) {
	this->tableOferte->clear();
	for (auto& oferta : oferte)
	{
		this->tableOferte->addItem(oferta.str().c_str());
...
void ConsoleGUI::gui_AddWishlist() {
	try {
		...
		reloadOfertaListFromWishlist(this->srv.getAllWishlist());
...
void ConsoleGUI::gui_AddWishlistmRandom() {
	try {
...
void ConsoleGUIWishlist::reloadOferteListFromWishlist(vector<Turism> oferte) {
	...
	this->tableWishlist->setRowCount(oferte.size());

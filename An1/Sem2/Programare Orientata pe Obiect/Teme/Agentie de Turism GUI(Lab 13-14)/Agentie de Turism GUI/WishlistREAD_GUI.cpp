...
//se ocupa de desenare

WishlistReadOnlyGUI::WishlistReadOnlyGUI(Wishlist& wishlist) : wishlist{ wishlist } {
	this->wishlist.addObserver(this);
	this->repaint();
}

void WishlistReadOnlyGUI::paintEvent(QPaintEvent* event) {
	QPainter g{ this };
	g.setPen(Qt::blue);
	...
	for (const auto& prog : this->wishlist.get_all()) {
		g.drawLine(x_coord(mt), y_coord(mt), width(mt), height(mt));
	}

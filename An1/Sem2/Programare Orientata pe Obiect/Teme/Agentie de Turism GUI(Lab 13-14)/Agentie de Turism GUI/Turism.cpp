...
bool Turism::eq(Turism a) {
	if (this->denumire == a.getDenumire() && this->destinatie == a.getDestinatie() && this->tip == a.getTip() && this->pret == a.getPret()) {
		return true;
	}
	return false;
}

std::string Turism::str() {
	std::string pretul = std::to_string(this->pret);
	return "Denumire: " + this->denumire + " - Destinatie: " + this->destinatie + " - Tip: " + this->tip + " - Pret: " + pretul;
}

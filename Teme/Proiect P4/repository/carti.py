...
class CartiMemoryRepository:
    ...
    def CMP_LUNGIME_DESCRIERE(self,a, b):
        # pt carti
    ...
    def partition(self,arr, low, high, key, reversed, cmp):
    ...
    def quickSort(self,arr, low, high, key, reversed, cmp):
        if len(arr) == 1:
            return arr
        if low < high:
            # pi is partitioning index, arr[p] is now
            # at right place
            pi = self.partition(arr, low, high, key, reversed, cmp)

            # Separately sort elements before
            # partition and after partition
            self.quickSort(arr, low, pi - 1, key, reversed, cmp)
            self.quickSort(arr, pi + 1, high, key, reversed, cmp)

    def sorted(self,list, key, reversed, cmp):
        # print(cmp)
        self.quickSort(list, 0, len(list) - 1, key, reversed, self.CMP_LUNGIME_DESCRIERE())
        return list
...
class CartiFileRepository:
    ...
    def __load_from_file(self):
        ...
        for line in lines:
            carte_id, carte_titlu, carte_descriere, carte_autor = [token.strip() for token in line.split(';')]
    ...
    def __save_to_file(self, all_carti):
        ...
        with open(self.__filename, 'w') as f:
            for carte in all_carti:
                carte_string = str(carte.getId()) + ';' + str(carte.getTitlu()) + ';' + str(carte.getDescriere()) + ';' + str(carte.getAutor()) + '\n'

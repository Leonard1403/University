...
class ClientiFileRepository:
    ...
    def __load_from_file(self):
        ...
        n = len(lines)
        el = 0
        for el in range(0, n, +3):
            # def __init__(self, id, nume, CNP):
            clienti_id = lines[el].strip()
            clienti_nume = lines[el + 1].strip()
            clienti_CNP = lines[el + 2].strip()
            clienti_id = int(clienti_id)
            clienti = Clienti(clienti_id,clienti_nume,clienti_CNP)
    ...
    def __save_to_file(self, all_clienti):
        with open(self.__filename, 'w') as f:
            for client in all_clienti:
                client_string = str(client.getId()) + '\n' + str(client.getNume()) + '\n' + str(client.getCNP()) + '\n'
